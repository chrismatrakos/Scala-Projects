...
object MonadicEvaluator {
  ...
  case class M[A](a: A) extends Monad[A, M] {
    def flatMap[B](k: A => M[B]): M[B] = k(a)

    def map[B](k: A => B): M[B] = M.unit(k(a))
  }
...
object BasicEvaluatorWithMonads {
  ...
  def evalAlt(term: Term) :M[Int] = term match {
    case Con (a) => M.unit (a)
    case Div (t,u) => for {
      a <- evalAlt(t)
      b <- evalAlt(u)
...
object ExceptionEvaluatorWithMonads {
  ...
  def eval (term :Term) :M[Int] = term match {
    case Con (a) => M.unit (a)
    case Div (t,u) => eval(u).flatMap(b => b match {
    		case  0 => Raise("devided by zero")
    		case _ => eval(t).map(a => a / b) } )
  }
...
// The basic  evaluator acts on terms, which is excessively simple.
// In the evaluator the  t and u are terms.
// The basic evaluator is simplicity itself.

// In the monad Evaluator on the other hand each of the variations has a similar structure
